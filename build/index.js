    ...
    try {
        switch (name) {
            case "manage_notebook": {
                const { action, notebook_id, title } = args;
                if (action === "list")
                    return { content: [{ type: "text", text: JSON.stringify(await ctx.client.listNotebooks()) }] };
                if (action === "get")
                    return { content: [{ type: "text", text: JSON.stringify(await ctx.client.getNotebook(notebook_id)) }] };
                if (action === "create")
                    return { content: [{ type: "text", text: JSON.stringify(await ctx.client.createNotebook(title)) }] };
                if (action === "rename")
                    return { content: [{ type: "text", text: JSON.stringify(await ctx.client.renameNotebook(notebook_id, title)) }] };
                if (action === "delete")
                    return { content: [{ type: "text", text: JSON.stringify(await ctx.client.deleteNotebook(notebook_id)) }] };
                if (action === "configure_chat") {
                    const { goal, custom_prompt } = args;
                    return { content: [{ type: "text", text: JSON.stringify(await ctx.client.configureChat(notebook_id, goal, custom_prompt)) }] };
                }
                break;
            }
            case "manage_source": {
                const { action, notebook_id, type, content, source_id, title } = args;
                if (action === "add") {
                    if (!type || !content)
                        throw new Error("Type and content are required for 'add' action");
                    const result = await ctx.client.addSource(notebook_id, type, content, title);
                    return { content: [{ type: "text", text: JSON.stringify(result) }] };
                }
                if (action === "rename") {
                    if (!source_id || !title)
                        throw new Error("Source ID and title are required for 'rename' action");
                    const result = await ctx.client.renameSource(notebook_id, source_id, title);
                    return { content: [{ type: "text", text: JSON.stringify(result) }] };
                }
                if (action === "delete") {
                    if (!source_id)
                        throw new Error("Source ID is required for 'delete' action");
                    const result = await ctx.client.deleteSource(notebook_id, source_id);
                    return { content: [{ type: "text", text: JSON.stringify(result) }] };
                }
                if (action === "sync") {
                    if (!source_id)
                        throw new Error("Source ID is required for 'sync' action");
                ...
                if (action === "check_freshness") {
                    ...
                    if (!ids || ids.length === 0)
                        throw new Error("source_ids array (or single source_id) is required for 'check_freshness' action");
            ...
            case "manage_studio": {
                ...
                if (action === "delete") {
                    if (!artifact_id)
                        throw new Error("Artifact ID is required for 'delete' action");
            ...
            case "query_notebook": {
                const { notebook_id, query, conversation_id } = args;
                const result = await ctx.client.query(notebook_id, query, conversation_id);
                return { content: [{ type: "text", text: JSON.stringify(result) }] };
            }
            case "authenticate": {
                const { method, cookies, csrfToken } = args;
                ...
                try {
                    ...
                    const authPath = path.join(home, ".notebooklm-mcp", "auth.json");
                    const authData = {
                        cookies: cookies.split(';').reduce((acc, curr) => {
...
async function main() {
    ...
    if (httpIndex !== -1) {
        // HTTP mode is now handled via the separate run-http.js helper script.
        // The StreamableHTTPServerTransport does not accept a 'port' option.
        // Use the helper script to start an HTTP server on the desired port.
        transport = new StreamableHTTPServerTransport();
        console.error("[INFO] HTTP transport initialized. Run the generated run-http.js script to bind a port.");
    }
    else {
