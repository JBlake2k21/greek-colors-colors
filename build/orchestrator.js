import { STUDIO_TYPES } from './constants.js';
export class NotebookOrchestrator {
    client;
    constructor(client) {
        this.client = client;
    }
    async performDeepWebResearch(notebookId, query, mode = 'deep') {
    ...
    async generateArtifact(notebookId, type, config) {
        ...
        switch (type) {
            case 'audio':
                studioType = STUDIO_TYPES.AUDIO;
                break;
            case 'video':
                studioType = STUDIO_TYPES.VIDEO;
                break;
            case 'quiz':
                studioType = STUDIO_TYPES.QUIZ;
                break;
            // study_guide could be slides or infographic depending on config, default to study_guide
            case 'slides':
                studioType = STUDIO_TYPES.STUDY_GUIDE;
                break;
            case 'infographic':
                studioType = STUDIO_TYPES.STUDY_GUIDE;
                break;
