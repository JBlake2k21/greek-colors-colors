...
import { RPC_IDS, STUDIO_TYPE_CODES } from './constants.js';
export class NotebookLMClient {
    ...
    async _buildRequestBody(rpcId, params) {
    ...
    _buildUrl(rpcId, path = '/') {
        this.reqIdCounter += 100000;
        const params = {
            rpcids: rpcId,
            'source-path': path,
            bl: this._currentBl || 'boq_labs-tailwind-frontend_20260121.08_p0',
            hl: 'en',
    ...
    _parseBatchResponse(responseText) {
    ...
    async configureChat(notebookId, goal = "default", customPrompt) {
    ...
    async pollResearch(notebookId, taskId) {
        ...
        const taskData = tasks.find((t) => Array.isArray(t) && t[0] === taskId);
        if (!taskData) {
            return { status: 'no_research' };
        }
    ...
    async importResearchSources(notebookId, taskId, sources) {
    ...
    async syncDriveSource(notebookId, sourceId) {
    ...
    async checkSourceFreshness(notebookId, sourceIds) {
    ...
    async addSource(notebookId, type, content, title) {
        let sourceData;
        if (type === 'text') {
    ...
    async renameSource(notebookId, sourceId, newTitle) {
    ...
    async listStudioArtifacts(notebookId) {
        ...
        if (res && Array.isArray(res)) {
            res.forEach((item) => {
                ...
                if (Array.isArray(item) && item.length > 2) {
                    ...
                    let type = "unknown";
                    if (typeCode === STUDIO_TYPE_CODES.AUDIO)
                        type = "audio";
                    if (typeCode === STUDIO_TYPE_CODES.VIDEO)
                        type = "video";
                    if (typeCode === STUDIO_TYPE_CODES.REPORT)
                        type = "report";
                    if (typeCode === STUDIO_TYPE_CODES.INFOGRAPHIC)
                        type = "infographic";
                    if (typeCode === STUDIO_TYPE_CODES.SLIDE_DECK)
                        type = "slide_deck";
                    if (typeCode === STUDIO_TYPE_CODES.FLASHCARDS)
                        type = "flashcards";
    ...
    async deleteStudioArtifact(notebookId, artifactId) {
        // RPC: V5N4be
    ...
    async query(notebookId, queryText, conversationId) {
    ...
    async createStudioArtifact(notebookId, type, config) {
